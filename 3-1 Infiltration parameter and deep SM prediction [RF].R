...
use_condaenv("r-reticulate")#Activate Conda Environment
py_config()
sklearn <- import("sklearn")# Import Python packages
np <- import("numpy")
config <- py_config()
config$numpy
time_cal<-Sys.time()
...
base_regressor <- RandomForestRegressor(n_estimators = as.integer(500), max_depth = as.integer(10),max_features="sqrt")
multi_output_model <- MultiOutputRegressor(base_regressor)
multi_output_model$fit(trainX, trainY)

##Use raster data to predict two parameters
...
# Convert the prediction results back to raster format
...
#Read the soil moisture of the day 0-5cm, change according to RF or GLM
path.SM.1km<-"~RF"
#path.SM.1km<-"~GLM"
SM.file <- dir(path.SM.1km, pattern = ".tif$", full.names = TRUE)
SM_0_5<-raster(SM.file[ss])
#Start using the SMAR model to predict the moisture content of the lower soil layer
#------Upper layer 0-5cm, lower layer 5-10cm---

s1<-SM_0_5/n_0_5#Upper layer saturation-- map
Zr1<-abs(0-5)
Zr2<-abs(5-10)
n1<-n_0_5 #Upper soil porosity--spatial map
n2<-n_5_10 #Porosity of the lower soil layer - spatial map
sc1<-sc1_sp#Predicted parameters sc1--spatial map
sw2<-sw2_sp #Predicted parameters sw2--spatial map

v.temp1<-1-SM_0_5/0.55
v.temp1@data@values[which(v.temp1@data@values<0)]<-0.01
v.temp2<--log(v.temp1)
V<-v.temp2/0.976 #Parameter V--spatial map
b=(n1*Zr1)/((1-sw2)*n2*Zr2) #Parameter b--spatial map
a_temp<-1/((1-sw2)*n2*Zr2)
a=V*a_temp #Parameter a--spatial map
...
#==========================================================================
#Parameter acquisition completed
#[Today, Tomorrow]
##S2_SMAR[[1]] is the value of t-1, S2_SMAR[[2]] is the value of t

if(ss==1){S2_SMAR<-stack(sc1,sc1)}

##Spatialization judgment function (0/1 variable)
judge.s1sc<-sc1
...
sm_SMAR<-S2_SMAR[[2]]*n2##This is the prediction result of the lower soil moisture
