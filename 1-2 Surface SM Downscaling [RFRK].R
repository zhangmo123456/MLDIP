...
#--Read and merge 36km environment variables (non-time-varying)
...
#Merge 1km other daily environmental variables
...
names_formula <- names(df)[c(-1,-2,-3)]
pre_formula<-paste("SM ~", paste(names_formula, collapse = " + "))
pre_formula <- as.formula(pre_formula)
Model <- randomForest(pre_formula, data =df, ntree=1000,proximity=TRUE,importance=TRUE)
...
residRF<-Model$predicted-df@data$SM

rstPixDF <- as(ec_raster_1km[[1]], "SpatialPixelsDataFrame")
df@data <- cbind(df@data, residual = residRF)
formMod <- residual ~ 1
